...
#define VICUT_DEBUG 1
   ...
   // generate annotation index to string hash table
   //map<int, string> annIdxToString;
   vector<string> annIdxToString(nuAnn);
   pair<string, int> pr1;
   unsigned int maxAnnStrLen = 0; // maximal annotation string length
   BOOST_FOREACH(pr1, uAnn)
   {
     if ( pr1.second < 0 )
       fprintf(stderr,"\n\nERROR at line %d: pr1.second < 0\n\n", __LINE__);

     annIdxToString[pr1.second] = pr1.first;
     if ( (pr1.first).length() > maxAnnStrLen )
       maxAnnStrLen = (pr1.first).length();
   }

   //fprintf(stderr,"\nmaxAnnStrLen: %d\n\n", maxAnnStrLen);

   fprintf(stderr,"--- Annotation index to string table\n");
   // pair<int, string> pr2;
   // BOOST_FOREACH(pr2, annIdxToString)
   // {
   //   fprintf(stderr,"%d\t%s\n", pr2.first, (pr2.second).c_str());
   // }

   for ( unsigned int j = 0; j < annIdxToString.size(); j++ )
   {
     fprintf(stderr,"%d\t%s\n", j, annIdxToString[j].c_str());
   }
   fprintf(stderr,"\n");

   // frequency table of annotation labels
   map<string, int> annFreq;
   for ( int i = 0; i < nLeaves; ++i )
   {
     if ( annIdx[i] > -1 )
     {
       annFreq[ annIdxToString[annIdx[i]] ]++;
     }
   }

   // printing frequency table to stdout
   fprintf(stderr,"--- Frequency table of annotation labels\n\n");
   BOOST_FOREACH(pr1, annFreq)
   {
     fprintf(stderr,"%*s:  %d\n",(maxAnnStrLen+1), (pr1.first).c_str(), pr1.second);
   }
   fprintf(stderr,"\n\n");
     ...
     if ( countMap[i] )
     {
       ...
       for ( int j = 0; j < nuAnn; ++j )
       {
         if ( AcountMap[i][j] )
         {
           ...
           fprintf(stderr,"%*s\t%d\t%d\t%.3f\n", maxAnnStrLen, annIdxToString[j].c_str(), AcountMap[i][j], Acount[j], h);
...
#if VICUT_DEBUG
           // label frequency total 2H(freq/total)
           fprintf(stderr,"%*s\t0\t%d\t0\n", maxAnnStrLen, annIdxToString[j].c_str(), Acount[j]);
#endif
         }
       }

#if VICUT_DEBUG
       fprintf(stderr,"%*s\t%d\t%d\t%.3f\t[= -H(p)]\n", maxAnnStrLen, "All", countMap[i], nAnnLeaves, -H(p));
#endif
     }
     else
     {
