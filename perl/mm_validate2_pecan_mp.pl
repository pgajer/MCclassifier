...
=head1 NAME
  ...
  mm_validate2_pecan_mp.pl

=head1 DESCRIPTION

  A multi-processor version of mm_validate2_pecan.pl.

  NOTE: its not working properly now as there is a race between different
  processes witting to an output file.

  To fix this ouput data should be stored in a hash table and printed to output
  at the end.
...
=head1 EXAMPLE
  ...
  mm_validate2_pecan_mp.pl --num-proc 8 --spp-file spp_missing_record.txt --max-no-nr-seqs 500 --perc-coverage 80
...
for my $phGr ( keys %phGrSppTbl )
{
  ...
  for my $spIdx ( 0..$#spp )
  {
    ...
    } ## end of  if ( @nrSeqIDs < $maxNumNRseqs ) {} else {}

    $pm->finish;

  } ## end of    for my $spIdx (0..$#spp )
