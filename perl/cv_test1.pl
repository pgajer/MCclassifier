...
=head1 EXAMPLE
  ...
  cv_test1.pl --cv-sp-size-thld 50 -i Firmicutes_group_5_V3V4
...
{
  my $nSpp = keys %spTbl;
  print "\n\nNo. of spp in the phylo group: $nSpp\n";
...
foreach my $i (0..($nFolds-1))
{
  ...
  for $offsetCoef ((0.9, 0.92, 0.94, 0.96, 0.98))
  {
    my $offsetStr = sprintf("o%d", int(100*$offsetCoef));
    my $cvReportFile = "$cvReportsDir/cv_report_$offsetStr" . ".txt";
    if ($skipErrThld)
    {
      $cvReportFile = "$cvReportsDir/cv_with_skip_error_thld_report_$offsetStr" . ".txt";
    }
    ...
    my $combTxFile = "$cvReportsDir/comb_$offsetStr" . ".tx";
    if ($i==0)
    {
...
print "\tReport writtent to $grDir/$cvReportsDir\n\n";
