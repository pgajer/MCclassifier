...
=head1 NAME
  ...
  multi_select_seqs_bigMem_v2.pl

=head1  DESCRIPTION

  A version of multi_select_seqs_bigMem.pl where each fasta file is put to its own directory
  phGr_dir/phGr.fa

=head1 SYNOPSIS

    multi_select_seqs_bigMem_v2.pl -s <file with 2 column table of seqIDs and taxonomies> -i <fastaFile>
...
=head1  EXAMPLES
  ...
  multi_select_seqs_bigMem_v2.pl --debug -t thld_5000_phGr_tbl.txt -i ../master_CX_HB_subRDP_sp100.fa
...
for my $sp ( keys %spIDsTbl )
{
  ...
  for ( @{ $spIDsTbl{$sp} } )
  {
    if ( !exists $faTbl{$_} )
    {
      warn "\n\n\tERROR: $_ does not exist in faTbl";
      print "\n\n";
      exit 1;
    }
    print OUT ">$_\n" . $faTbl{$_} . "\n";
