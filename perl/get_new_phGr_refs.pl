...
=head1 NAME
  ...
  get_new_phGr_refs.pl

=head1 DESCRIPTION

  Given data generated by mm_validate_pecan.pl, generate a tab delimited file
  with a table consisting of the following 4 columns:

  <seqID> <species> <100%-identity cltr size> <phylo-group>

  The sequences come from clusters containing one named species and query
  sequences. Both query and ref sequences of the given phylo-group are present in
  the table. The reason for this is that some ref seq's will undergo change of
  taxonomy when we encounter a cluster like this all small number species will be
  renamed in that cluster to Lactobacillus_helveticus_crispatus.

  Renaming will not be done when there are two large species groups within a
  cluster.

  Cluster c117 (325)
  139         Lactobacillus_sp_3:                 2
  140         Lactobacillus_sp_21:                1
  141         Lactobacillus_sp_8:                 1
  142         Lactobacillus_sp_2:                 1
  143         Lactobacillus_helveticus_crispatus: 209
  144         Lactobacillus_sp_1:                 3
  145         Lactobacillus_sp_13:                1
  146         Lactobacillus_sp_5:                 2
  147         Lactobacillus_sp_4:                 2
  148         Lactobacillus_sp_6:                 2
  149         Lactobacillus_acidophilus:          1
  150         Lactobacillus_sp_12:                1
  151         NA:                                 99

  There are two important parameters:

  min-cltr-size: add a sequence to the list if its cluster size is at least that
                 specified by the flag --min-cltr-size.

  max-num-sp-refs: maximum number of ref sequences of a given species that we
                   want to be present in the final phylo-group. Thus, we add no
                   more than the difference between max-num-sp-refs and the
                   current number of ref seq's of the given species in the given
                   phylo-group.
...
=head1 EXAMPLE
  ...
  get_new_phGr_refs.pl --debug -d mm_may12_validate_pecan_dir2 -o mm_may12_phylo_clfy_dir2
...
{
  print "\n\n\tERROR: Missing validation directory\n\n";
  pod2usage(verbose => 2,exitstatus => 0);
  exit 1;
}
...
my $trainTxFile = $outDir . "/ref_seq_tbl.txt"; # columns: seqID, species, phyloGroup, multiplicity
