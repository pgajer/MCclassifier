...
=head1 NAME
  ...
  phylo_split_from_tree.pl

=head1 DESCRIPTION

  The aim of the script is to generate a split of all bacterial (and Archeal)
  sequences into phylogenetically sound sub-groups from a consensus tree.

=head1 SYNOPSIS

  phylo_split_from_tree.pl -i <master fa file> -t < master tx file> -d <tree file> -o <output dir> [Options]
...
=head1 EXAMPLE
  ...
  phylo_split_from_tree.pl -d genus_consensus_majority_vote.tree -i master_V3V4_no_outliers.fa -t master_V3V4_no_outliers.tx -o phylo_split_from_ge_cons_tree_dir
...
run_vicut_no_query( $treeFile, $geOrFile, $orVicutDir );
...
sub get_cltrSizes_labels
{
  ...
  my %geClTbl  = read_tbl( $txCltrFile );
  ...
  for my $ge ( keys %geClTbl )
  {
    my $cl = "c" . $geClTbl{$ge};
    $clSize{ $cl }  += $sizeTbl{ $ge };
    $clSizeR{ $cl } += $sizeTblR{ $ge };
  }
  ...
  {
    my $cl = "c" . $geClTbl{$ge};
    $cltrSizeTbl{$ge} = $cl . "__" . $clSize{ $cl };
    $cltrSizeTblR{$ge} = $cl . "__" . $clSizeR{ $cl };
  }
