...
//----------------------------------------------------------------- openFasta ----
// initializes a pointer to MSArec_t and reads the first MSA record
MSArec_t * openFasta(char *seqfile)
{
  MSArec_t *rp = MallocOrDie(sizeof(MSArec_t));
...
int readMSArecord( MSArec_t *rp )
{
  ...
  int n = 0;
  int seenNuc = 0; // =1 on the first non-gap character
  int gap;
  rp->seqStart = 0;
  rp->ngSeqLen = 0;
