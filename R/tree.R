...
source("/Users/pgajer/organizer/programming/R/libs/queue.R")
## https://www.r-bloggers.com/implementing-a-queue-as-a-reference-class/
source("/Users/pgajer/organizer/programming/R/libs/misc.R") # for all.same()

## Find the first common ancestral node of seq.ids
first.common.ancestor <- function( tr, seq.ids )
{
    if ( length(seq.ids) == 1 )
    {
        id <- seq.ids[1]
        if ( is.character(id) )
        {
            node <- tree.find( tr, id )
        } else {
            node <- id
        }

        p <- tree.parent.node( tr, node )
    } else {
        max.depth <- 0
        for ( id in seq.ids )
        {
        ...
        paths <- matrix(nrow=length(seq.ids), ncol=max.depth)
        for ( i in seq(seq.ids) )
        {
            id <- seq.ids[i]
            if ( is.character(id) )
            {
            ...
            path <- tree.path.to.root( tr, node )
            paths[i,seq(path)] <- path
        }

        i <- 1
        while ( i <= max.depth && all.same(paths[,i]) )
        {
            i <- i+1
        }
...
#' Returns the path from the node to the root of the tree
#'
#' @param tr input phylo object
#' @return node index
#' @export
## Author: PG
tree.path.to.root <- function( tr, node )
{
    if ( length(node) > 1 )
    {
        stop("The second argument must be of length 1")
    }

    if ( is.character(node) )
    {
        node <- which( tr$tip.label == node )
        if ( length(node) == 0 )
        {
            stop("Node not present in the tree")
        }
    }

    path <- c(node)
    while ( (p <- tree.parent.node( tr, node )) != -1 )
    {
        path <- c(p, path)
        node <- p
    }
...
#' Returns a vector of leaf indices for the sub-tree spannded by the given node
#'
#' @param tr input phylo object
#' @return node index
#' @export
...
##
## Given a node in the tree, the routine gives indices of leaves of the subtree
## of tr rooted at node.idx.
node.leaves <- function( tr, node )
{
    if ( length(node) > 1 )
    {
        stop("The second argument must be of length 1")
    ...
    while ( bfs$size() )
    {
        ...
        if ( is.leaf.phylo( tr, node ) )
        {
            leaves <- c(leaves, node )
        } else {
            for ( idx in tree.child.nodes( tr, node )[[1]][[1]] )
            {
                bfs$push( idx );
            }
        }
